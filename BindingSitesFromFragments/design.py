...
def fuzzball_composition_design(ligand_conformer_path, match_path, match_residue_map, params_path,
                                ...
                                rotset_limit=200, apply_minimization=False, dalphaball_path=None, match_cst=None):
    """
    ...
    if use_complementary_rotsets:
        ...
        viable_rotamers = generate_fuzzball_contact_rotamersets(ligand_conformer_path, match_path, match_pose, sfxn, match_residue_map,
                                                                flag_special_rot=True, rotset_limit=rotset_limit,
    ...
    if dalphaball_path:
        ...
        hoels_filter = rosetta.protocols.simple_filters.HolesFilter()
        hoels_filter.set_residue_selector(relevant_positions_rs)
    ...
    for i in range(nstruct):
        ...
        if match_cst:
            ...
            rosetta.protocols.rosetta_scripts.XmlObjects.create_from_string(make_bundle_xml).get_mover("add_match_constraints").apply(design_pose)
        ...
        if dalphaball_path:
            holes_value = hoels_filter.compute(design_pose)
            print(f'HOLES: {holes_value}')
