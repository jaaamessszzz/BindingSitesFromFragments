...
class Generate_Motif_Residues():
    ...
    def score_residue_ligand_interactions(self):
        ...
        residue_ligand_interactions_dir = os.path.join(self.user_defined_dir, 'Inputs', 'Residue_Ligand_Interactions')
        os.makedirs(residue_ligand_interactions_dir, exist_ok=True)

        score_interactions_list_path = os.path.join(residue_ligand_interactions_dir, 'PDBs_to_score.txt')
        open(score_interactions_list_path, 'w').close()

        # Import ligand pdb as generated by molfile_to_params.py (for loop if conformer library has been generated)
        rosetta_inputs_path = os.path.join(self.user_defined_dir, 'Inputs', 'Rosetta_Inputs')
        for conformer in pdb_check(rosetta_inputs_path):
            ligand_ID = os.path.basename(os.path.normpath(conformer)).split('.')[0]
            ligand_prody = prody.parsePDB(conformer)

            conformer_residue_output_dir = os.path.join(residue_ligand_interactions_dir, ligand_ID)
            ...
            for motif_residue in pdb_check(os.path.join(self.user_defined_dir, 'Representative_Residue_Motifs')):
                motif_residue_name = os.path.basename(os.path.normpath(motif_residue)).split('.')[0]
                ...
                with open(score_interactions_list_path, 'a+') as pdb_list:
        ...
        run_jd2_score = subprocess.Popen(['~/Rosetta/main/source/bin/score_jd2.macosclangrelease',
                                          '-l',
                                          score_interactions_list_path,
                                          '-extra_res_fa',
                                          os.path.join(rosetta_inputs_path, '{}.params'.format(current_ligand))
                                          ])
        run_jd2_score.wait()
