...
class Generate_Constraints():
    ...
    def BW_constraints_from_gurobi_solutions(self, gurobi_solutions_csv, conformer, use_solution_residues_only=True):
        ...
        for solution in gurobi_solutions['Residue_indicies']:
            ...
            # Add tuple of (fragment, cluster) to cluster_list_set
            cluster_list_set.add(tuple([(row['fragment'], row['cluster']) for index, row in relevant_rows.iterrows()]))
        ...
        # Generate matcher xml for each
        # pprint.pprint(cluster_list_set)
        # print(len(cluster_list_set))
        # print(cluster_residue_set)
        # print(len(cluster_residue_set))

        single_pose_prody = prody.parsePDB(os.path.join(self.single_pose_dir, '{}-single_pose.pdb'.format(conformer)))
        fragment_cluster_groups = self.res_idx_map_df.groupby(['fragment', 'cluster'])
        ...
        for solution_tuples in cluster_list_set:
            ...
            for solution_tuple in solution_tuples: # (fragment, cluster)
                ...
                for cluster_residue in residue_row_indices:
                    ...
                    constraint_atoms_dict = self.determine_constraint_atoms(single_pose_prody, conformer, residue_index)
